...
def _prune_fail_cooldowns(now: float | None = None) -> None:
    """Drop expired unlock/sudomode fail timestamps so the dict cannot grow forever."""
    now = time.time() if now is None else now
    dead = [uid for uid, until in _unlock_fail_until.items() if until <= now]
    for uid in dead:
        _unlock_fail_until.pop(uid, None)


def _fail_cooldown_remaining(uid: int) -> float:
    now = time.time()
    _prune_fail_cooldowns(now)
    return max(0.0, _unlock_fail_until.get(uid, 0.0) - now)


def _mark_auth_fail(uid: int) -> None:
    now = time.time()
    _prune_fail_cooldowns(now)
    _unlock_fail_until[uid] = now + _FAIL_COOLDOWN_S
...
async def _handle_dm(message: discord.Message) -> None:
    ...
    if low.startswith("unlock ") or low.startswith("!unlock "):
        left = _fail_cooldown_remaining(uid)
        if left > 0:
            await message.reply(f"Too many failed attempts. Wait ~{int(left) + 1}s.")
            return
...
async def _dispatch_command(
    ...
    if cmd == "__HISTORY__":
        ...
        for e in entries:
            ...
            cmd_shown = truncate(str(e.get("command", "")), 80)
            uname_shown = truncate(str(e.get("user_name", "?")), 24)
            lines.append(f"{ts} {uname_shown}: {cmd_shown}{rc_s}")
        lines.append("```")
        await message.reply(fit_discord_message("\n".join(lines)))
        return
    ...
    if cmd == "__LAST__":
        ...
        if not entry:
            await message.reply("No history yet.")
            return
        shown_cmd = safe_code_block(truncate(str(entry.get("command") or ""), 500))
        shown_user = truncate(str(entry.get("user_name") or "?"), 40)
        tail = (
